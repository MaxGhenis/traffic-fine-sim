...
class FixedLaborAgent(Agent):
    ...
    def optimize(
        ...
        result = minimize_scalar(speeding_objective, bounds=(0, 1), method="bounded")
...
def compare_with_without_labor_response(
    ...
    society_flat_endo = Society(
        agents_flat_endo, fine_structure_flat, tax_rate, death_prob_factor, vsl
    )
    results["flat_endogenous"] = society_flat_endo.simulate(max_iterations)

    # 2. Flat fine with fixed labor
    agents_flat_fixed = [FixedLaborAgent(inc, fixed_labor_supply) for inc in incomes]
    society_flat_fixed = Society(
        agents_flat_fixed, fine_structure_flat, tax_rate, death_prob_factor, vsl
    )
    ...
    society_income_endo = Society(
        agents_income_endo, fine_structure_income, tax_rate, death_prob_factor, vsl
    )
    results["income_endogenous"] = society_income_endo.simulate(max_iterations)

    # 4. Income-based fine with fixed labor
    agents_income_fixed = [FixedLaborAgent(inc, fixed_labor_supply) for inc in incomes]
    society_income_fixed = Society(
        agents_income_fixed, fine_structure_income, tax_rate, death_prob_factor, vsl
    )
    ...
    decomposition["transfer_effect_flat"] = results["flat_endogenous"]["ubi"] * len(
        incomes
    )
    decomposition["transfer_effect_income"] = results["income_endogenous"]["ubi"] * len(
        incomes
    )
    decomposition["transfer_difference"] = (
        decomposition["transfer_effect_income"] - decomposition["transfer_effect_flat"]
    )
    ...
    return {
        ...
        "summary": {
            ...
            "optimal_without_labor_response": (
                "Income-based" if decomposition["pure_deterrence"] > 0 else "Flat"
            ),
            "optimal_with_labor_response": (
                "Income-based" if decomposition["total_effect"] > 0 else "Flat"
            ),
        },
    }
