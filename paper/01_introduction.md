...
Our baseline calibration using US CPS data finds that income-based fines generate higher utilitarian welfare than flat fines in 95% of Monte Carlo draws. While the double distortion mechanism is present---income-based fines raise effective marginal tax rates for regular speeders by 0.2--2 percentage points---its magnitude is small relative to the deterrence equity gain. The mean welfare difference favors income-based fines ($\Delta W = 0.83$, 95% CI: $[-0.02, 3.22]$), and income-based fines reduce consumption inequality (Gini 0.325 vs.\ 0.343).

The result is robust to the labor supply elasticity: our quadratic disutility specification implies a Frisch elasticity of 1.0, four times the meta-analytic central estimate of 0.25, yet income-based fines still dominate. At the empirically estimated elasticity, the welfare advantage would be even larger. The welfare ranking is reinforced under inequality-averse social welfare functions, with income-based fines dominating even more strongly under Rawlsian criteria.
